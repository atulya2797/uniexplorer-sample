...
class InstitutionSearchController extends Controller {
     ...
     */
    public function institutionListing() {
        $compactData = $this->getInstitutionSearchData();
        ...
        if (\Request::ajax()) {
            return view('basic.allInstitutionList', $compactData);
        }

        return view('basic.institutionListing', $compactData);
    }
    ...
    public function getInstitutionSearchData() {
        //intake filter
        list($currentYearDetail, $nextYearDetail) = $this->getIntakeFilterInstitutionSearch();
        // Duration filter
        $months = $this->getDurationFilterInstitutionSearch();
        //levelofstudy filter
        ...
        $input = Input::all();
        $user = $this->student;
        $search = isset($input['institution_name']) ? $input['institution_name'] : '';
...
//        check if all selected
        $allInstitutions = Institution::has('getInstitutionAdmin')
                ...
                ->whereHas('getBranch', function($getBranch) use ( $subdiscipline, $location, $tuitionFeesPA, $tuitionFeesEP, $intakeCurrentYear, $intakeNextYear, $user, $levelofstudy, $duration) {
            $getBranchReturn = $getBranch->whereHas('getintake', function($getIntake) use ($subdiscipline, $tuitionFeesPA, $tuitionFeesEP, $intakeCurrentYear, $intakeNextYear, $user, $levelofstudy, $duration) {
                $getIntakeReturn = $getIntake->whereHas('getCourseData', function($getCourseData) use ($subdiscipline, $user, $levelofstudy, $duration) {
                    ...
                    if ($duration) {
                        $getCourseData = $getCourseData->whereIn('duration', $duration);
                    }
                    if ($subdiscipline) {
        ...
        return compact('allInstitutions', 'search', 'user', 'tutionFeesFilter', 'cities', 'subdisciplines', 'currentYearDetail', 'nextYearDetail', 'countAllInstitutions', 'levelOfStudy','months');
    }

    public function getDurationFilterInstitutionSearch() {
        $user = $this->student;
        ...
        foreach ($months as $mkey => $month) {
            $allInstitutions = Institution::has('getInstitutionAdmin')
                    ...
                    ->where(['type' => Institution::TYPE_UNIVERSITY])
                    ->whereHas('getBranch', function($getBranch) use ($user,$month) {
                $getBranchReturn = $getBranch->whereHas('getintake', function($getIntake) use ($user,$month) {
                    $getIntakeReturn = $getIntake->whereHas('getCourseData', function($getCourseData) use ($user,$month) {
                        $getCourseData = $getCourseData->where([
                            'visibility' => Courses::PUBLISHED,
                            'approval' => Courses::APPROVED
                        ]);
                    ...
                    //check if course intake deadline is reached
                    if ($month->duration) {
                            $getIntakeReturn = $getIntakeReturn->where('duration', '<=', $month->duration);
                        }
                    $getIntakeReturn = $getIntakeReturn->where('applicationDeadlineDate', '>', date("Y-m-d"));
                    return $getIntakeReturn;
                });
            ...
            $months[$mkey]['totalCourses'] = $allInstitutions->count();
        }
        return $months;
    }
